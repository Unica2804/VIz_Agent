...
def data_parser(file_path_or_url: str,tool_context:ToolContext, file_type: str ) -> Dict:
    ...
    Returns:
        dict: Metadata including the artifact filename for the parsed JSON.
    """
    ...
    try:
        ...
        json_str= json.dumps(parsed_data["data"], ensure_ascii=False, indent=2,default=str)
        json_bytes= json_str.encode("utf_8")

        # Create ADK Part
        artifact_part=types.Part.from_bytes(data=json_bytes,mime_type="application/json")

        # Generate a filename
        base_name=Path(file_path_or_url).stem if not is_url else "Downloaded_file"
        ...
        return {
            ...
            "columns": parsed_data.get("columns", 0),
        }
...
def _parse_xml(file_path: str) -> Dict:
    ...
    columns = (
        len(rows_data[0]) if rows_data and isinstance(rows_data[0], dict) else len(data)
    )
...
def _parse_json(file_path: str) -> Dict:
    ...
    return {"data": data, "pages": 1, "rows": rows, "columns": columns}
